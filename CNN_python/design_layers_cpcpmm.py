...
class design:
	...
	def __init__(self, img_ch, img_h, img_w,output_dim,class_layers=class_layers):
		...
		self.n_flt1, self.flt1_h, self.flt1_w, self.stride1, self.pad1 =10, 3, 3, 1, 1
		...
		self.n_flt2, self.flt2_h, self.flt2_w, self.stride2, self.pad2 =5, 3, 3, 2, 1
...
# -- 順伝播 --
	...
	def forward_propagation(self,x,batchsize):
		self.n_bt = batchsize
		#rgb dataset.pyのプログラム中__init__で入力したイメージをnp.arrayに変換するので　height,width,ch になる　array_img=np.asarray(img_resize)
		#更に、trans_img=array_img.transpose(2,0,1)でch, hight, widthの順に入れ替えたデータセット(CNNでは先頭次元がチャネル数なので）にしている。これが画像1枚あたりのデータ形式
		#これを一旦、img_one_dim = np.ravel(trans_img)でch.h,wを1次元配列に変換する。（これは、rgb_dataset.pyでnormalize処理をしやすくするための対応）
		#次にimg_list.append(img_one_dim)で、1次元配列画像を画像枚数分配列化（2次元配列）している　なのでデータ形式は最終的に　[画像サンプル数, (channel, hight, weightをフラット化)]
		#のような2次元配列になっている
...
# -- 逆伝播 --
	def backpropagation(self,given_output):
		...
		self.grad_img = self.ml_1.grad_x.reshape(self.n_bt, self.pl_2.y_ch, self.pl_2.y_h, self.pl_2.y_w)
