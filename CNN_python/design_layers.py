...
class design:
	...
	def __init__(self, img_ch, img_h, img_w,output_dim,class_layers=class_layers):
		...
		self.ml_1 = class_layers.MiddleLayer(self.n_fc_in,self.hidden_nodes)
		self.ol_1 = class_layers.OutputLayer(self.hidden_nodes,self.output_dim)
...
# -- 順伝播 --
	...
	def forward_propagation(self,x,batchsize):
		...
		self.ol_1.forward(self.ml_1.y)

# -- 逆伝播 --
	def backpropagation(self,given_output):
		self.t = given_output
		self.ol_1.backward(self.t)
		self.ml_1.backward(self.ol_1.grad_x)
